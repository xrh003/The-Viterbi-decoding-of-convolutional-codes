...
survive* viterbidecoder(int demodule[length][N],int* recover,int* generate)
{
	...
	{//生成第i个时间段内的幸存路线片段
		if(i<M)//对应篱笆图左部下落区
		{
			...
			{//生成第j状态对应的幸存路线
				int temp1=(j>>(M-i))&(statenum/2-1),temp2=(j>>(M-1-i))&1;
				int temp3=dist(myfence[temp1].outcode[temp2],demodule[i-1]);
				mysurvive[j].sufferstate[i]=myfence[temp1].out[temp2]->currentstate;
				mysurvive[j].optimizer+=temp3*temp3;
				mysurvive[j].hamingdist+=temp3;
		...
		{//此篱笆图区域内每个状态必由2条路径到达,取汉明距离较小的为此时间段内此状态的幸存路径
			...
			for(j=0;j<statenum;j++)
			{
				...
				mysurvive[j].outcode[i-1]=j&1;mysurvive[j].sufferstate[i]=j;
				if((0==k)||((2==k)&&(rand()&1==0)))
				{//node1的距离小
					tempi=dist(code1,demodule[i-1]);
					copysur1(mysurvive,j,mysurvivenew,node1->currentstate,i);//赋予新路径
					mysurvive[j].optimizer+=tempi*tempi;
					mysurvive[j].hamingdist+=tempi;
				}
				else
				{//node2的距离小
					tempi=dist(code2,demodule[i-1]);
...
{//将路径数组mysurvive中每个路径的前i个数据赋给mysurvivene中的每个路径
	for(int j=0;j<statenum;j++)
	{
		...
		mysurvivenew[j].optimizer=mysurvive[j].optimizer;
		for(int k=l;k<i;k++)
		{
...
{//将路径surv2的前i个数据赋给surv1,变成新的surv1
	...
	surv1[m].optimizer=surv2[n].optimizer;
